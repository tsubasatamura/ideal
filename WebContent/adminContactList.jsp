...
<link rel="stylesheet" type="text/css" media="all" href="css/tamura2.css"/>
<title>お客様お問い合わせ一覧</title>
<!-- スクリプト・JS -->
<!--[if lt IE 9]>
...
<% } %>
<form id="listform" name="listform" action="AdminInsertSvl" method="post">
<%if(list.size()!=0){ %>
<table id="adlist">
<tr class="index">
<td id="namae">お名前</td>
...
<tr>
<td id="namae"><%=list.get(i).getUsr_name() %>様</td>
<td id="naiyou"><%=list.get(i).getMessage() %></td>
<td id="hiduke"><%=list.get(i).getDate().substring(0,11) %></td>
<td id="hensin" class="submit"><button id="send" name="userID" value="<%=list.get(i).getUsr_id()%>">回答する</button></td>
</tr>
<%} %>
</table>
<%}else{ %>
