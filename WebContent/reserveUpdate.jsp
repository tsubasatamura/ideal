...
<!-- コンテンツ -->
	...
	<div id="wrapper">
		...
		<h1 align="center"><jsp:getProperty name="userInfo" property="usr_name" />様
			ご予約内容変更<%=title%>
		</h1>
		<%=msg%>
		<div align="center" class="reserveInsert">
			<form id="frm01" name="crform" method="post" action="ReserveUpdateSvl">
				<input type="hidden" name="person" value="" /> <input type="hidden"
					name="rsvYyMmDd" value="" /> <input type="hidden" name="courseId" value="" />
				<input type="hidden" name="charter" value="<%=charter%>" /> <input
					type="hidden" name="rsvId" value="<%=reserve.getRsvId()%>" />
			</form>
			<label>予約番号</label>
			<div><%=reserve.getRsvId()%></div>
			<form id="cusfrm" name="cusfrm" method="post" action="ReserveOperationSvl"
				...
				<div>
					<input type="hidden" name="rsvId" value="<%=reserve.getRsvId()%>" /> <input
						type="hidden" name="mode" value="<%=ReserveOperationSvl.UPDATE%>" /> <input
						type="submit" class="btn" value="変更" />
				</div>
			</form>
		</div>
	</section>
